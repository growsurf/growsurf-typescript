...
export class AccountResource extends APIResource {
   ...
   * response. The key is locked until the team owner's email address is verified. Resend
   * the verification email with `client.team.resendOwnerVerificationEmail()`, then
   * retry. Accounts whose email is never verified are deleted automatically after 7
   * days. The API key is rotated the first time the account owner signs in to the
   * GrowSurf dashboard.
   *
   * @example
   * ```ts
