...
export interface ClientOptions {
   ...
   *
   * Defaults to process.env['GROWSURF_BASE_URL'].
   */
   ...
   * Set the log level.
   *
   * Defaults to process.env['GROWSURF_LOG'] or 'warn' if it isn't set.
   */
...
/**
 * API Client for interfacing with the Growsurf API.
 */
export class Growsurf {
  apiKey: string;
   ...
   * API Client for interfacing with the Growsurf API.
   *
   * @param {string | undefined} [opts.apiKey=process.env['GROWSURF_API_KEY'] ?? undefined]
   * @param {string} [opts.baseURL=process.env['GROWSURF_BASE_URL'] ?? https://api.growsurf.com/v2] - Override the default base URL for the API.
   ...
   */
  constructor({
    baseURL = readEnv('GROWSURF_BASE_URL'),
    ...
    this.logLevel =
      ...
      parseLogLevel(readEnv('GROWSURF_LOG'), "process.env['GROWSURF_LOG']", this) ??
      defaultLogLevel;
  ...
  #baseURLOverridden(): boolean {
    return this.baseURL !== 'https://api.growsurf.com/v2';
  }
  ...
  static Growsurf = this;
  static DEFAULT_TIMEOUT = 60000; // 1 minute

  static GrowsurfError = Errors.GrowsurfError;
